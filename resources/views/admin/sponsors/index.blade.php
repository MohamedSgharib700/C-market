...
@section('content')
            ...
            <div class="row">
                <div class="col-6">
                    <div class="page-header">
                        <h4 class="page-title">{{trans('sponsors')}}</h4>
                    </div>
                </div>
            </div>
            <!--page-header closed-->
                                ...
                                <div class="col-md-2">
                                    <span style="" class="table-add ">
                                            ...
                                            <a href="{{ route('admin.sponsors.create') }}"
                                               class="btn btn-add btn-block">{{trans('add_sponsor')}}</a>
                                    ...
                                    <table id="example-2"
                                        ...
                                        @foreach ($list as $sponsor)
                                            <tr>
                                                <td>{{ $sponsor->id }}</td>
                                                 <td>
                                                   {{ $sponsor->translate('ar')->name }}
                                                </td>
                                                <td>{{ $sponsor->translate('en')->name }}</td>
                                         
                                                <td><img style="width: 100px" src="{{ asset($sponsor->image) }}"></td>
                                               
                                                <td>
                                                    <label class="custom-switch">
                                                        <input type="checkbox" name="active" id="active"
                                                               data-model="{{  get_class($sponsor) }}"
                                                               data-id="{{ $sponsor->id }}"
                                                               value="{{ $sponsor->active }}"
                                                               {{ $sponsor->active ? 'checked' : '' }} class="custom-switch-input">
                                                        ...
                                                        <div class="btn-group dropdown">
                                                            ...
                                                            <div class="dropdown-menu">
                                                                    ...
                                                                    <a class="dropdown-item has-icon"
                                                                       href="{{ route('admin.sponsors.edit', ['sponsor' => $sponsor->id]) }}"><i
                                                                    ...
                                                                    <button type="button" class="dropdown-item has-icon"
                                                                            data-toggle="modal"
                                                                            data-target="#exampleModalLong{{ $sponsor->id }}">
    ...
    @foreach ($list as $sponsor)
        <!-- Message Modal -->
            <div class="modal fade" id="exampleModalLong{{ $sponsor->id }}" tabindex="-1" role="dialog"
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <form action="{{ route('admin.sponsors.destroy', ['sponsor' => $sponsor]) }}" method="Post">
                            @method('DELETE')
                            @csrf
